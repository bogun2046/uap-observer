...
from .reddit import RedditSourceExtractor
...
def _extractor_for(name: str, media_type: str) -> Extractor:
    ...
    if name == PdfExtractor.name:
        return PdfExtractor()
    if name == FeedXmlExtractor.name and base_media_type in {
        "application/atom+xml",
        "application/rss+xml",
        "application/xml",
        "text/xml",
    }:
        return FeedXmlExtractor()
    if (
        name == RedditSourceExtractor.name
        and base_media_type in RedditSourceExtractor.supported_media_types
    ):
        return RedditSourceExtractor()

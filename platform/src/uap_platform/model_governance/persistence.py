...
_NON_RETRYABLE_ERROR_CODES = frozenset(
    {
        ...
        "monthly_model_budget_exhausted",
...
class PostgresModelGovernanceStore:
    ...
    def monthly_cost_microunits(self, observed_at: datetime, currency: str = "CNY") -> int:
        """Return conservative calendar-month spend from existing minor-unit records."""
    ...
    def _job_result(
        ...
        if execution.error_code not in _NON_RETRYABLE_ERROR_CODES and _retryable_http_status(
            execution.http_status, execution.error_code
        ):
            return (
                "retryable_failure",

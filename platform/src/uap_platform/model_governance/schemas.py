...
class SummaryOutput(StrictModel):
    language_code: Literal["zh-CN"] = "zh-CN"
...
class ClassificationOutput(StrictModel):
    ...
    suggested_document_category: Literal[
        "official_report",
        "government_document",
        "military",
        "scientific_research",
...
class ClaimCandidate(StrictModel):
    ...
    claim_type: Literal["observation", "attribution", "event", "assessment", "other"]
    assertion_status: Literal["reported", "disputed", "unverified"]
    evidence: list[EvidenceLocator] = Field(min_length=1, max_length=20)

...
V1_DEEPSEEK_MONTHLY_BUDGET_MICRO_CNY = 20_000_000
...
class ModelJobHandler:
    ...
    def __init__(
        ...
        *,
        deepseek_monthly_budget_microunits: int = V1_DEEPSEEK_MONTHLY_BUDGET_MICRO_CNY,
    ) -> None:
        if deepseek_monthly_budget_microunits < 0:
            raise ValueError("DeepSeek monthly budget cannot be negative")
        self._store = store
        self._providers = providers
        self._deepseek_monthly_budget_microunits = deepseek_monthly_budget_microunits
    ...
    def handle(
        ...
        try:
            ...
            if request.provider == "deepseek" and self._store.monthly_cost_microunits(
                datetime.now(UTC), "CNY"
            ) >= self._deepseek_monthly_budget_microunits:
                return self._persist_governance_failure(
                    job_id,
                    lease_token,
                    request,
                    code="monthly_model_budget_exhausted",
                    summary="DeepSeek monthly budget exhausted",
                    http_status=402,
                    started_at=datetime.now(UTC),
                )
        ...
        response_micro_cost = (
            ...
            if response.cost_microunits is not None
            else response.cost_minor_units * 10_000
        )
        if request.provider == "deepseek" and (
            self._store.monthly_cost_microunits(started_at, "CNY") + response_micro_cost
            > self._deepseek_monthly_budget_microunits
        ):
            execution = ModelExecution(
                request=request,
                ...
                validation_status=ValidationStatus.INVALID,
                response=response,
                error_code="monthly_model_budget_exhausted",
                error_summary="DeepSeek monthly budget exhausted",
                http_status=402,
                started_at=started_at,
                finished_at=finished_at,
            )
        ...
        return self._store.persist_and_finish_job(job_id, job_attempt_id, lease_token, execution)
...
def _retryable_http_status(http_status: int | None, error_code: str) -> bool:
    if http_status in (401, 403):
        return False
    return (
        http_status in (408, 429, 504)
        or (http_status is not None and http_status >= 500)
        or (error_code in {"timeout", "deadline_exceeded", "connection_reset"})
    )

    ...
    def run(
        ...
        *,
        ...
        collector = RssCollector(
            self._fetch,
            lambda items: self._store.persist_items(source_id, run_id, items, self._clock()),
        )
        try:
            ...
            if self._health_tracker is not None:
                self._health_tracker.record(source_id, result, self._clock(), self._source_policy)
            ...
            if self._health_tracker is not None:
                self._health_tracker.record(source_id, failure, self._clock(), self._source_policy)
            self._store.fail_source_run_and_job(
                run_id,
                job_id,
                attempt_id,

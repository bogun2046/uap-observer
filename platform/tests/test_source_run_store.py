...
class FakeCursor:
    ...
    def execute(self, query: str, parameters: object = None) -> None:
        if "ops.require_active_source_job_lease" in query and not self.source_run_lease_valid:
    ...
    def fetchone(self) -> tuple[object, ...] | None:
        ...
        if "SELECT id" in self.last_query and "canonical_url" in self.last_query:
            return (
                (uuid.UUID("00000000-0000-7000-8000-000000000023"),) if self.existing_url else None
            )
...
def test_store_finish_accepts_non_success_source_run(status: int) -> None:
    ...
    store = PostgresSourceRunStore(connection, FakeObjectClient())  # type: ignore[arg-type]
    classification = (
        FetchClassification.NOT_MODIFIED if status == 304 else FetchClassification.TRANSIENT_FAILURE
    )

...
def test_build_request_dispatches_versioned_adapters() -> None:
    ...
    _, pdf_extractor = build_extraction_request(payload("application/pdf", PdfExtractor.name))
    _, srt_extractor = build_extraction_request(payload("text/srt", SrtExtractor.name))

...
def test_record_review_decision_calls_definer() -> None:
    ...
    case_id = uuid.uuid4()
    assert (
        record_review_decision(connection, case_id, "approve", "approve this claim") == decision_id
    )
...
def test_structured_changes_code_is_frozen() -> None:
    assert REVIEW_STRUCTURED_CHANGES_UNSUPPORTED == ("review_structured_changes_unsupported")

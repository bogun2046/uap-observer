...
def test_build_knowledge_bundle_partial() -> None:
    accepted = AcceptedCandidate(
        ...
        rejected_locators=(RejectedLocator(locator_ordinal=1, reason_code="locator_out_of_range"),),
    )
...
def test_build_attempt_metrics_counts() -> None:
    accepted = AcceptedCandidate(
        ...
        rejected_locators=(RejectedLocator(locator_ordinal=1, reason_code="locator_duplicate"),),
    )
    rejected = RejectedCandidate(
        ordinal=1,
        reason_code="knowledge_locator_unmappable",
        rejected_locators=(RejectedLocator(locator_ordinal=0, reason_code="locator_out_of_range"),),
    )
    report = MappingReport(
        classification=MappingClass.MATERIALIZABLE,
        anchor=ExtractionAnchor(status=AnchorStatus.MATCHED, extraction_id=uuid.uuid4()),
...
def test_failure_metrics_arithmetic() -> None:
    ...
    assert (
        metrics["materialized_locators"] + metrics["rejected_locators"] == metrics["input_locators"]
    )

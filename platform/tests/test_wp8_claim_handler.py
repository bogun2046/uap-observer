...
def test_worker_claim_one_none_and_dispatch_rejects_other_types() -> None:
    ...
    inactive = ResolveClaimsWorker(conn, MagicMock(), worker_id="pre", claims_handler_active=False)
    ...
    with pytest.raises(KnowledgePayloadError):
        worker.dispatch((uuid.uuid4(), uuid.uuid4(), "resolve_claims", "not-mapping", uuid.uuid4()))

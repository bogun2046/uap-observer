...
class FakeStore:
    ...
    def monthly_cost_microunits(self, *_args: object) -> int:
        return self.monthly_cost_micro
...
def test_handler_persists_valid_result_without_logging_input() -> None:
    ...
    handler = ModelJobHandler(
        cast(Any, store),
        ProviderRegistry(
            {"static": StaticProvider({"summary": "A safe summary", "bullets": ["one"]})}
        ),
    )
...
def test_provider_rate_limit_maps_to_retryable_failure() -> None:
    store = FakeStore()
    handler = ModelJobHandler(cast(Any, store), ProviderRegistry({"static": FailingProvider()}))
...
def test_deepseek_monthly_budget_stops_call_before_provider() -> None:
    store = FakeStore()
    store.monthly_cost_micro = 20_000_000
    provider = SequenceProvider([])
    handler = ModelJobHandler(
        cast(Any, store), ProviderRegistry({"deepseek": provider})
    )
    ...
    handler.handle(
        ...
        ATTEMPT_ID,
        TOKEN,
        {**model_payload(), "provider": "deepseek", "model": "deepseek-flash"},
    )

    assert provider.calls == 0
    assert store.executions[0].error_code == "monthly_model_budget_exhausted"


def test_retryable_failure_is_not_reused_and_next_attempt_calls_provider_again() -> None:
    store = FakeStore()
    ModelJobHandler(cast(Any, store), ProviderRegistry({"static": FailingProvider()})).handle(
        JOB_ID, ATTEMPT_ID, TOKEN, {**model_payload(), "provider": "static"}
    )
    ModelJobHandler(
        cast(Any, store),
        ProviderRegistry({"static": StaticProvider({"summary": "ok", "bullets": ["one"]})}),
    ).handle(
        uuid.uuid4(),
        uuid.uuid4(),
        uuid.uuid4(),
    ...
    classified, errors = validate_output(
        ModelTaskType.CLASSIFICATION,
        {
            "relevance": {
                "decision": "relevant",
                "confidence": 0.9,
                "reason": "The source discusses UAP policy.",
            },
            "labels": ["policy"],
            "suggested_document_category": "government_document",
        },
    )
...
def test_handler_maps_unknown_provider_and_unexpected_provider_failure() -> None:
    ...
    store = FakeStore()
    handler = ModelJobHandler(cast(Any, store), ProviderRegistry({"broken": BrokenProvider()}))
    handler.handle(
        JOB_ID,
        ATTEMPT_ID,
        TOKEN,

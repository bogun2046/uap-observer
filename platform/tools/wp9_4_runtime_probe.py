...
def g9_18(
    ...
    state, primary = sqlerror_tx(api, [*bind(reviewer, uuid.uuid4()), select_sql(invalid_id)])
    require("g9-18 invalid state", state, "22023")
    require("g9-18 invalid code", primary, "review_selection_not_valid")
    state, primary = sqlerror_tx(api, [*bind(reviewer, uuid.uuid4()), select_sql(summary_id)])
    require("g9-18 summary state", state, "22023")
    require("g9-18 summary code", primary, "review_selection_type_unsupported")
    state, primary = sqlerror_tx(api, [*bind(reviewer, uuid.uuid4()), select_sql(pending_id)])
...
def g9_19(
    ...
    _document, _analysis, candidate_id = seed_pending_candidate(admin, uuid.uuid4().hex[:12], name)
...
def g9_36(
    ...
    require(
        ...
        int(scalar(admin, "SELECT count(*) FROM core.entities WHERE canonical_name = %s", name)),
        entities_before + 1,
    )
    require(
        "g9-36 accept events",
    ...
    replay_bound = call_api(api, reviewer, bind_request, bind_sql(bind_candidate, target))
    ...
    state, primary = sqlerror_tx(api, [*bind(reviewer, bind_request), bind_sql(other_bind, target)])
...
def extra_cases(
    ...
    missing = uuid.uuid4()
    state, primary = sqlerror_tx(api, [*bind(reviewer, uuid.uuid4()), select_sql(missing)])
    require("extra missing selection state", state, "23503")
    require("extra missing selection code", primary, "review_selection_missing")
    classification_id = seed_typed_analysis(admin, uuid.uuid4().hex[:12], "classification")
    state, primary = sqlerror_tx(
    ...
    state, primary = sqlerror_tx(api, [*bind(reviewer, uuid.uuid4()), accept_sql(uuid.uuid4())])
    ...
    state, primary = sqlerror_tx(api, [*bind(reviewer, uuid.uuid4()), accept_sql(no_evidence)])
    ...
    execute(admin, "SET session_replication_role = origin")
    state, primary = sqlerror_tx(api, [*bind(reviewer, uuid.uuid4()), accept_sql(origin_candidate)])
    ...
    state, primary = sqlerror_tx(api, [*bind(reviewer, uuid.uuid4()), accept_sql(rejected)])
    ...
    state, primary = sqlerror_tx(api, [*bind(reviewer, uuid.uuid4()), accept_sql(resolved)])
    ...
    state, primary = sqlerror_tx(api, [*bind(reviewer, uuid.uuid4()), bind_sql(resolved, target)])
...
def _activity_wait_event(admin: psycopg.Connection[Any], application_name: str) -> str | None:
    with admin.cursor() as cursor:
        cursor.execute(
            """

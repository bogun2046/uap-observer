...
def assert_frozen_canonical(admin: psycopg.Connection[Any]) -> None:
    ...
    sql_number = scalar(admin, "SELECT audit._canonical_json(%s::jsonb)", FROZEN_NUMBER_SOURCE)
    ...
    sql_number_sha = scalar(admin, "SELECT audit._payload_sha256(%s::jsonb)", FROZEN_NUMBER_SOURCE)
    ...
    sql_array = scalar(admin, "SELECT audit._canonical_json(%s::jsonb)", FROZEN_NUMBER_ARRAY_SOURCE)
    ...
    sql_prec = scalar(admin, "SELECT audit._canonical_json(%s::jsonb)", FROZEN_PRECISION_SOURCE)

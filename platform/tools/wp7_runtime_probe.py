...
def make_retry_available(administrator: psycopg.Connection[Any], job_id: uuid.UUID) -> None:
...
def claim(connection: psycopg.Connection[Any], expected_job_id: uuid.UUID) -> tuple[Any, ...]:
    with connection.cursor() as cursor:
        cursor.execute(

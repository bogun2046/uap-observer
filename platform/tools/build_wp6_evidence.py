"""Build deterministic WP6 evidence and a separated-anchor checksum manifest."""
...
EVIDENCE_REQUIRED = (
    ...
    "platform/src/uap_platform/documents/__init__.py",
...
def main() -> None:
    ...
    if missing:
        raise SystemExit(f"missing WP6 evidence files: {', '.join(missing)}")
    rows = [
        {
    ...
    report.write_text(
        json.dumps(
            {"freeze": "G6-FROZEN-20260819-01", "files": rows},
            ensure_ascii=False,
            indent=2,
            sort_keys=True,
        )
    ...
    print(
        json.dumps(
            {"files": len(rows), "manifest_sha256": sha256_file(manifest)},
            sort_keys=True,
        )
    )

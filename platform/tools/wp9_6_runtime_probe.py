...
def evidence_snapshot(
    ...
    return [(uuid.UUID(str(row[0])), uuid.UUID(str(row[1])), str(row[2])) for row in rows]
...
def g9_replace_preserves_nonsupport(
    ...
    claim_id = call_api(
        ...
        author,
        uuid.uuid4(),
        manual_sql(document_id, f"{MANUAL_TEXT} replace {tag}", "witness", [support_span]),
    )
    ...
    context_id = insert_typed_evidence(admin, claim_id, context_span, document_id, "context")
    ...
    require("replace empty token", empty_primary, "review_structured_changes_unsupported")

...
def claim_grants(admin: psycopg.Connection[Any], claim_id: uuid.UUID) -> list[tuple[Any, ...]]:
...
def g9_35(
    ...
    closed_at = scalar(admin, "SELECT closed_at FROM audit.review_cases WHERE id = %s", case_id)
    require(
        "g9-35 events",
        int(
            scalar(
...
def extra_cases(
    ...
    replay_payload = '{"schema":"publication-outbox.v1","grant_id":"' + str(uuid.uuid4()) + '"}'
    ...
    try:
        ...
        require("extra outbox conflict code", primary, "review_idempotency_payload_conflict")
    finally:
...
def main() -> None:
    ...
    try:
        ...
        g9_29_30(admin, api, reviewer, extras[7][1], seed_subjects(admin, uuid.uuid4().hex[:8])[1])
        ...
        extra_cases(admin, api, reviewer, other, senior, document_id, claim_id, entity_id)
    finally:
        api.close()
        admin.close()
    print(

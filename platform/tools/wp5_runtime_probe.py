...
def claim_expected(
    ...
    if claimed_job_id != expected_job_id:
        raise RuntimeError(f"claim_job returned {claimed_job_id}; expected {expected_job_id}")
...
def main() -> None:
    ...
    with connection.cursor() as cursor:
        cursor.execute("SELECT outcome FROM ingest.source_runs WHERE job_id = %s", (job_id,))
            ...
            with connection.cursor() as cursor:
                cursor.execute(
                    ...
                    (
                        ...
                        config_id,
                        cross_source_job,
                        f"wp5-runtime-cross-source-{probe_key}",
                        now,
                    ),
                )

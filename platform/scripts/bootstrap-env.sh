...
v1_local_admin_token=$(openssl rand -hex 32)
...

{
    ...
    grep -q '^UAP_V1_LOCAL_ADMIN_TOKEN=' "$env_file" 2>/dev/null || printf 'UAP_V1_LOCAL_ADMIN_TOKEN=%s\n' "$v1_local_admin_token"
    grep -q '^UAP_V1_LIBRARY_PORT=' "$env_file" 2>/dev/null || printf '%s\n' 'UAP_V1_LIBRARY_PORT=8091'
} >"$tmp_file"

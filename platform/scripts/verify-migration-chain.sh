...
legacy_head=0019_manual_claims_binding
...
alembic_step -x role=migrator upgrade "$legacy_head"
alembic_step -x role=migrator upgrade "$legacy_head"
...
alembic_step -x role=migrator downgrade 0016_review_decisions_and_grants
test "$(query "SELECT version_num FROM public.alembic_version")" = "0016_review_decisions_and_grants"
test "$(query "SELECT EXISTS (SELECT 1 FROM pg_proc WHERE proname='select_analysis_result' AND pronamespace = 'audit'::regnamespace)")" = "f"
test "$(query "SELECT EXISTS (SELECT 1 FROM pg_proc WHERE proname='record_review_decision' AND pronamespace = 'audit'::regnamespace)")" = "t"
test "$(query "SELECT EXISTS (SELECT 1 FROM pg_indexes WHERE schemaname='audit' AND indexname='uq_document_grant_live')")" = "t"
alembic_step -x role=migrator upgrade "$legacy_head"
test "$(query "SELECT version_num FROM public.alembic_version")" = "0019_manual_claims_binding"
...
if failclosed_output=$($compose run --rm --no-deps --env "UAP_DATABASE_URL=$failclosed_url" \
    object-store-init alembic -x role=migrator upgrade 0013_entity_merge_state_machine 2>&1); then

...
alembic_step -x role=migrator upgrade head
test "$(query "SELECT version_num FROM public.alembic_version")" = "0012_entity_materialization"
...
if failclosed_output=$($compose run --rm --no-deps --env "UAP_DATABASE_URL=$failclosed_url" \
    object-store-init alembic -x role=migrator upgrade 0012_entity_materialization 2>&1); then
...
echo "Migration chain verified: 0001 -> 0002 -> 0003 -> 0004 -> 0005 -> 0006 -> 0007 -> 0008 -> 0009 -> 0010_knowledge_foundation -> 0011_claim_materialization -> 0012_entity_materialization, idempotent head, 0012 roundtrip, fail-closed backfill, downgrade smoke."

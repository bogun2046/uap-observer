...
- 实施起点：本设计 docs-only 提交 `G9_DESIGN_SHA`
- 状态：设计待冻结；实施与验收尚未开始
- 技术审核：Codex
- 阶段放行：项目负责人
...
| `G9-GATE-9.2` | WP9.2 | G9-06–G9-09 | grant、selection、merge 包装 | 可授权 WP9.3 |
| `G9-GATE-9.3` | WP9.3 | G9-10–G9-16 | candidate 晋升、merge 包装、手工 Claim | 可授权 WP9.4 |
| `G9-GATE-9.4` | WP9.4 | G9-17–G9-19 | merge 包装、手工 Claim、CI 全接线 | 可授权 WP9.5 |
| `G9-GATE-9.5` | WP9.5 | G9-20–G9-22 | 手工 Claim、orchestrator | 可授权 WP9.6 |
| `G9-GATE-9.6` | WP9.6 | G9-23–G9-26 及全量复跑 | 公开投影 | 可签署 G9 |

阶段通过只表示“技术上具备进入下一阶段条件”，不会自动授权实施者继续。

## 4. 重点判定标准

1. 伪造 principal 参数或缺少 GUC 不得成功写入。
2. `uap_worker` 不得打开 case 或记录决定。
3. 任一成功 approve 必须有 grant 与 outbox，且无 `public` 行、无 `publish_*` job。
4. `core.merge_entities` 对 `uap_api` 仍是 `42501`。
5. G8-16C 在 WP9 全阶段保持 `dead` / `terminal_failure` / `knowledge_relation_task_not_in_wp8`。

...
## 补充安全断言 G3-S01：迁移器失败收口

- 新增日期：2026-08-13（Asia/Shanghai）
- 新增原因：首轮独立复验发现部署结束后 `uap_migrator` 仍可登录；本断言只提高安全要求，不降低冻结预期。
- 步骤：在独立临时数据库停留于 0003，故意破坏 0004 前置对象后通过正式部署脚本升级。
- 预期：迁移非零退出、revision 仍为 0003、事务不留下半迁移对象，且 EXIT 清理后 `uap_migrator.rolcanlogin=false`。
